...
const OrderCorrection = () => {
  ...
  const fetchOrders = async () => {
    ...
    try {
      ...
      setOpen(false); setActive(null); setDraft(null)
    } catch (e) {
      console.error(e)
  ...
  // เมื่อเปิดดูรายละเอียด จากโหมดไหน ให้จัดรูปแบบค่าเริ่มต้นให้เหมาะกับการแก้
  const openDrawer = async (row) => {
    setRowError(""); setTouched(new Set()); setEditing(false)
    setActive(row)

    // พยายามเดา payment & editor
    ...
    if (mode === "buy") {
      setDraft({
        ...
        klang_location: "",
        payment_id: "",
        humidity: "", impurity: "", gram: "",
        comment: "",
        // เครดิต (ถ้าเลือกชำระเป็นเครดิต)
        dept_allowed_period: "",
        dept_postpone: false,
        dept_postpone_period: "",
      })
    } else {
      // SELL: ต้องรวมแถว sub_order 1/2
  ...
  const buildProductSpecIn = (specId) => {
    const raw = specOptions.find(o => String(o.id) === String(specId))?.spec
    if (!raw) return null
    return {
  ...
  const save = async () => {
    ...
    const putIfTouched = (c, k, v) => { if (touchedKeys.has(k)) c[k] = v }

    if (draft.kind === "buy") {
      const c = {}
      if (touchedKeys.has("spec_id")) {
        const s = buildProductSpecIn(draft.spec_id)
        if (s) c.spec = s
      }
      putIfTouched(c, "order_serial", draft.order_serial || undefined)
      putIfTouched(c, "entry_weight", draft.entry_weight === "" ? undefined : Number(cleanDecimal(draft.entry_weight)))
      putIfTouched(c, "exit_weight", draft.exit_weight === "" ? undefined : Number(cleanDecimal(draft.exit_weight)))
      putIfTouched(c, "weight", draft.weight === "" ? undefined : Number(cleanDecimal(draft.weight)))
      putIfTouched(c, "price_per_kilo", draft.price_per_kilo === "" ? undefined : Number(cleanDecimal(draft.price_per_kilo)))
      putIfTouched(c, "price", draft.price === "" ? undefined : Number(cleanDecimal(draft.price)))
      putIfTouched(c, "humidity", draft.humidity === "" ? undefined : Number(cleanDecimal(draft.humidity)))
      putIfTouched(c, "impurity", draft.impurity === "" ? undefined : Number(cleanDecimal(draft.impurity)))
      putIfTouched(c, "gram", draft.gram === "" ? undefined : Number(onlyDigits(draft.gram)))
      putIfTouched(c, "date", draft.date ? new Date(draft.date).toISOString() : undefined)
      putIfTouched(c, "branch_location", draft.branch_location ? Number(draft.branch_location) : undefined)
      putIfTouched(c, "klang_location", draft.klang_location ? Number(draft.klang_location) : undefined)
      putIfTouched(c, "payment_id", draft.payment_id ? Number(draft.payment_id) : undefined)
      putIfTouched(c, "comment", draft.comment || undefined)
      ...
      const payload = {
        ...
        changes: c,
        dept: buildDept(draft, true) || undefined,
      }

      try {
        await apiAuth(`/order/orders/buy/${draft.order_id}`, { method: "PATCH", body: payload })
        await fetchOrders()
        setEditing(false); setOpen(false)
      } catch (e) {
        // เผื่อกรณี id เป็น sell จริง ให้ fallback
        try {
          await apiAuth(`/order/orders/sell/${draft.order_id}`, { method: "PATCH", body: payload })
          await fetchOrders()
          setEditing(false); setOpen(false)
        } catch (e2) {
          console.error(e2)
          setRowError(e2?.message || "บันทึกไม่สำเร็จ")
        }
      } finally {
        setSaving(false)
      }
      ...
      putIfTouched(c, "order_serial_1", draft.order_serial_1 || undefined)
      putIfTouched(c, "order_serial_2", draft.order_serial_2 || undefined)
      putIfTouched(c, "license_plate_1", draft.license_plate_1 || undefined)
      putIfTouched(c, "license_plate_2", draft.license_plate_2 || undefined)
      putIfTouched(c, "weight_1", draft.weight_1 === "" ? undefined : Number(cleanDecimal(draft.weight_1)))
      putIfTouched(c, "weight_2", draft.weight_2 === "" ? undefined : Number(cleanDecimal(draft.weight_2)))
      putIfTouched(c, "price_1", draft.price_1 === "" ? undefined : Number(cleanDecimal(draft.price_1)))
      putIfTouched(c, "price_2", draft.price_2 === "" ? undefined : Number(cleanDecimal(draft.price_2)))
      putIfTouched(c, "price_per_kilo", draft.price_per_kilo === "" ? undefined : Number(cleanDecimal(draft.price_per_kilo)))
      putIfTouched(c, "gram", draft.gram === "" ? undefined : Number(onlyDigits(draft.gram)))
      putIfTouched(c, "date", draft.date ? new Date(draft.date).toISOString() : undefined)
      putIfTouched(c, "branch_location", draft.branch_location ? Number(draft.branch_location) : undefined)
      ...
      const payload = {
        meta: { edited_by: editorId, reason: (draft.reason || "").trim() || undefined },
        changes: c,
        dept: buildDept(draft, false) || undefined,
      }
      ...
      try {
        ...
        // เผื่อกรณี id เป็น buy จริง ให้ fallback
        try {
          await apiAuth(`/order/orders/buy/${draft.order_id}`, { method: "PATCH", body: payload })
          await fetchOrders()
          setEditing(false); setOpen(false)
    ...
    <div className="min-h-screen bg-white text-black dark:bg-slate-900 dark:text-white rounded-2xl">
      <div className="mx-auto max-w-7xl p-4 md:p-6">
        <div className="mb-4 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
          ...
          {/* Toggle Buy/Sell — ให้หน้าตาเหมือนหน้า Order */}
      ...
      {/* DRAWER: ดูรายละเอียด / แก้ */}
      <div className={`fixed inset-0 z-50 ${open ? "pointer-events-auto" : "pointer-events-none"}`} aria-hidden={!open}>
        <div className={`absolute inset-0 bg-black/60 transition-opacity ${open ? "opacity-100" : "opacity-0"}`} onClick={closeDrawer} />
        <div className="absolute inset-y-0 right-0 flex w-full max-w-4xl transform flex-col bg-white text-black shadow-2xl transition-all dark:bg-slate-800 dark:text-white"
             style={{ translate: open ? "0 0" : "100% 0" }}>
          {/* Header */}
          <div className="flex items-center justify-between border-b border-slate-200 px-5 py-4 dark:border-slate-700">
            <div className="text-lg md:text-2xl font-semibold">
              {!draft ? "รายละเอียดออเดอร์" :
                draft.kind === "sell"
                  ? `รายละเอียดออเดอร์ขาย #${draft.sale_id || draft.order_id}`
                  : `รายละเอียดออเดอร์ซื้อ #${draft.order_serial || draft.order_id}`
              }
            </div>
            <div className="flex items-center gap-2">
              {!editing ? (
                ...
                <>
                  <button
                    ...
                    onClick={save}
                    disabled={saving}
                    className="rounded-2xl bg-emerald-600 px-5 py-2 text-sm font-semibold text-white hover:bg-emerald-700 disabled:opacity-60"
                  >
                    {saving ? "กำลังบันทึก..." : "บันทึกการแก้ไข"}
                  </button>
                  <button
                    type="button"
                    onClick={() => { setEditing(false); openDrawer(active) }}
                    className="rounded-2xl border border-slate-300 px-5 py-2 text-sm hover:bg-slate-50 dark:border-slate-600 dark:hover:bg-slate-700"
                  >
                    ยกเลิก
                  </button>
                </>
              ...
              <button
                type="button"
                onClick={closeDrawer}
                className="rounded-2xl border border-slate-300 px-4 py-2 text-sm hover:bg-slate-50 dark:border-slate-600 dark:hover:bg-slate-700"
              >
                ปิด
              </button>
            </div>
          ...
          <div className="flex-1 overflow-y-auto p-4 md:p-6">
            {!draft ? (
              ...
              <>
                {rowError && (
                  <div className="mb-4 rounded-xl border border-red-200 bg-red-50 p-3 text-sm text-red-700 dark:border-red-400 dark:bg-red-900/20 dark:text-red-200">
                    {rowError}
                  </div>
                )}

                {/* กลุ่ม: ผู้แก้ไข / เหตุผล */}
                <div className="mb-5 grid gap-4 md:grid-cols-3">
                  <div className="rounded-2xl border border-slate-200 bg-white/70 p-4 dark:border-slate-700 dark:bg-slate-700/40">
                    ...
                    {!editing ? (
                      <div className="rounded-xl border border-slate-200 bg-slate-50 px-4 py-2.5 text-sm dark:border-slate-700 dark:bg-slate-700/60">
                        {draft.edited_by || "-"}
                      </div>
                    ) : (
                      <>
                        {/* ล็อกโดยระบบ: ดึงจากบัญชีผู้ใช้ปัจจุบัน */}
                        <input
                          inputMode="numeric"
