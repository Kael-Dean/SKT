              ...
              <tr>
                ...
                {MONTHS.map((m, mi) => (
                  <Fragment key={`sub-${m.key}`}>
                    {unitCols.map((u) => (
                      <th key={`${m.key}-${u.id}-sub`} className={cx("px-2 py-2 text-sm font-semibold", monthStripeCell(mi))}>
                        {u.short}
                      </th>
                    ))}
                  </Fragment>
              ...
              {items.map((p, idx) => {
                ...
                return (
                  <tr key={`row-${pid}`}>
                    ...
                    <td className={cx("px-2 py-2 text-center font-semibold", idx % 2 ? STRIPE.cellOdd : STRIPE.cellEven)} style={{ width: COL_W.unit }}>
                      {p.unit || "-"}
                    </td>
