...
import { apiAuth } from "../lib/api" // ✅ helper แนบโทเคน/BASE URL อัตโนมัติ

/** ---------- Utils (ยกสไตล์เดียวกับ Order.jsx) ---------- */
...
/** ---------- Reusable ComboBox (เทียบ Order.jsx) ---------- */
function ComboBox({
  options = [],
  value,
  onChange,
...
/** ---------- Helpers: RBAC & user ---------- */
const ALLOWED_ROLES = new Set(["mng", "admin", "aadmin", "HA", "ha"])
function getCurrentUser() {
  // พยายามอ่านจาก localStorage รูปแบบที่พบบ่อย
  try {
    const userJson =
    ...
    // เดาโครงที่พบบ่อย: { id, roles:[], role, position }
    const id =
      Number(u?.id ?? u?.user_id ?? u?.userid ?? u?.personnel_id ?? u?.personId ?? 0) || null
    const rawRoles =
      u?.roles ||
      u?.permissions ||
      (u?.role ? [u.role] : []) ||
      (u?.position ? [u.position] : [])
    const roles = new Set((Array.isArray(rawRoles) ? rawRoles : []).map((r) => String(r || "").trim()))
    return { id, roles }
  } catch {
    return { id: null, roles: new Set() }
  }
}
function hasPrivilege(rolesSet) {
  for (const r of rolesSet) {
    if (ALLOWED_ROLES.has(String(r || "").toLowerCase())) return true
  }
  return false
}
...
const OrderCorrection = () => {
  /** ---------- RBAC ---------- */
  const { id: currentUserId, roles } = getCurrentUser()
  const allowed = hasPrivilege(roles)

  /** ---------- Dates (default: เดือนนี้) ---------- */
  ...
  const [specDict, setSpecDict] = useState({}) // id:string -> raw spec row (ใช้สร้าง ProductSpecIn)
    ...
    const loadInitial = async () => {
      try {
        const [branches, specs, payB, payS] = await Promise.all([
          ...
          apiAuth(`/order/form/search`),   // รายการสำเร็จรูป (ProductSpec.prod_name + คอลัมน์ spec ทั้งชุด)
  ...
  /** ---------- Fetch orders (ยึด /order/orders/report ของ BE) ---------- */
  ...
  useEffect(() => { if (filters.q.length >= 2 || filters.q.length === 0) fetchOrders() }, [debouncedQ]) // auto on search
  ...
  /** ---------- EDIT MODAL ---------- */
  const [open, setOpen] = useState(false)
  const [active, setActive] = useState(null)  // แถวที่เลือกจากตาราง
  const [draft, setDraft] = useState(null)    // แบบฟอร์มแก้ไข
  const [editing, setEditing] = useState(false)
  const [saving, setSaving] = useState(false)
  const [rowError, setRowError] = useState("")
  const [touched, setTouched] = useState(new Set()) // key ที่ผู้ใช้แก้ => จะใส่ลง PATCH เท่านั้น
  ...
  const openModal = async (row) => {
    ...
    setEditing(false)
    setActive(row)
    // เดาตามข้อมูลแถว: ถ้า entry/exit > 0 ค่อนข้างเป็น buy; ไม่แน่ใจไม่เป็นไร เราจะลองยิง PATCH buy ก่อน ถ้า 404 ค่อยลอง sell
    const guessType = (toNumber(row.entry_weight) > 0 || toNumber(row.exit_weight) > 0) ? "buy" : "sell"

    // พยายามเติม branch/klang โดยเทียบชื่อ
    const { branchId, klangId } = await tryPrefillBranchKlang(row)

    setDraft({
      order_id: row.id,
      type: guessType,                       // ให้ผู้ใช้สลับได้ถ้าจำเป็น
      edited_by: currentUserId || "",        // ต้องเป็น number
      reason: "",
      ...
      payment_id: "", // เดาไม่ได้จาก /report — ให้ผู้ใช้เลือกเองถ้าต้องการแก้
      comment: "",

      // buy-ish fields (จาก /report มีเฉพาะ entry/exit/weight/price/unit_price)
  ...
  const buildProductSpecIn = (specId) => {
    ...
    if (!raw) return null
    // map ตามสเปก ProductSpecIn ของ BE
    return {
  ...
  const buildDept = (d, isBuy) => {
    // เงื่อนไข: buy(เครดิต=4) | sell(เครดิต=2)
  ...
  const save = async () => {
    ...
    // ลำดับความพยายาม: ถ้าเลือก type=buy จะลอง buy ก่อน ถ้า 404 ค่อยลอง sell (และกลับกัน)
    ...
    try {
      ...
      setEditing(false)
      setOpen(false)
      await fetchOrders() // ดึงรายการใหม่เพื่อรีเฟรชตัวเลขแถว
    } catch (e) {
      console.error(e)
      ...
      <div className="min-h-screen bg-white text-black dark:bg-slate-900 dark:text-white rounded-2xl">
        <div className="mx-auto max-w-4xl p-6">
          <div className="rounded-2xl border border-red-200 bg-red-50 p-6 text-red-800 dark:border-red-400 dark:bg-red-900/10 dark:text-red-200">
            <div className="text-2xl font-semibold mb-2">403 — ไม่มีสิทธิ์เข้าหน้านี้</div>
            <p>หน้านี้อนุญาตเฉพาะสิทธิ์ <b>mng / admin / HA</b> เท่านั้น</p>
          </div>
        </div>
      </div>
    )
  ...
  return (
    <div className="min-h-screen bg-white text-black dark:bg-slate-900 dark:text-white rounded-2xl">
      <div className="mx-auto max-w-7xl p-4 md:p-6">
        <h1 className="mb-4 text-2xl font-bold text-gray-900 dark:text-white">🛠️ แก้ไขออเดอร์ (สำหรับบัญชี)</h1>

        {/* Filters — เหมือนหน้า Order.jsx */}
              ...
              {!active || !draft ? (
                ...
                <>
                    ...
                    <div className="rounded-2xl border border-slate-200 bg-white/70 p-4 dark:border-slate-700 dark:bg-slate-700/40 md:col-span-2">
                      ...
                      {!editing ? (
                        ...
                        <input
                          ...
                          placeholder="เช่น แก้ไขเลขที่ใบสำคัญผิด/แก้จำนวนเงิน ฯลฯ"
                        />
                      )}
                    </div>
                  </div>
                  ...
                  <div className="mb-6 grid grid-cols-1 gap-5 md:grid-cols-3">
                    <div>
                      <label className="mb-1 block text-sm text-slate-700 dark:text-slate-300">ชนิดออเดอร์</label>
                      {!editing ? (
                        <div className="rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-base dark:border-slate-700 dark:bg-slate-700/60 capitalize">
                          {draft.type}
                        </div>
                      ) : (
                        <div className="flex gap-3">
                          {[
                            { key: "buy", label: "ซื้อ (Buy)" },
                            { key: "sell", label: "ขาย (Sell)" },
                          ].map(opt => (
                            <label key={opt.key} className="inline-flex items-center gap-2">
                              <input
                                type="radio"
                                className="h-4 w-4"
                                checked={draft.type === opt.key}
                                onChange={() => { setD({ type: opt.key }); touch("type") }}
                              />
                              <span>{opt.label}</span>
                            </label>
                          ))}
                    ...
                    <div>
                      <label className="mb-1 block text-sm text-slate-700 dark:text-slate-300">วันที่เอกสาร</label>
                      {!editing ? (
                        <div className="rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-base dark:border-slate-700 dark:bg-slate-700/60">
                        ...
                        <DateInput
                          value={draft.date}
                          onChange={(e) => { setD({ date: e.target.value }); touch("date") }}
                        />
                      )}
                    </div>

                    <div>
                      <label className="mb-1 block text-sm text-slate-700 dark:text-slate-300">ฟอร์มสำเร็จรูป (Spec)</label>
                      {!editing ? (
                        <div className="rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-base dark:border-slate-700 dark:bg-slate-700/60">
                          {active?.species || "-"} {/* โชว์ชื่อคร่าว ๆ จากแถวรายงาน */}
                        </div>
                      ) : (
                        <ComboBox
                          options={specOptions}
                      ...
                      <div className="mt-1 text-xs text-slate-500 dark:text-slate-400">แมปเข้า <code>ProductSpecIn</code> อัตโนมัติเมื่อบันทึก</div>
                    </div>

                    <div>
                      <label className="mb-1 block text-sm text-slate-700 dark:text-slate-300">วิธีชำระ</label>
                      {!editing ? (
                        <div className="rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-base dark:border-slate-700 dark:bg-slate-700/60">
                          {draft.payment_id ? `#${draft.payment_id}` : "-"}
                        </div>
                      ) : (
                        <ComboBox
                    ...
                    <div className="md:col-span-3">
                      <label className="mb-1 block text-sm text-slate-700 dark:text-slate-300">หมายเหตุ (comment)</label>
                      {!editing ? (
                        <div className="rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-base dark:border-slate-700 dark:bg-slate-700/60">
                          {draft.comment || "-"}
                        </div>
                  ...
                  {/* เงื่อนไขเครดิต (AP/AR) */}
                  <div className="mb-6 rounded-2xl border border-indigo-200 bg-indigo-50/60 p-4 dark:border-indigo-400 dark:bg-indigo-900/10">
                    <div className="mb-2 text-base font-semibold text-indigo-800 dark:text-indigo-200">🧾 เงื่อนไขเครดิต</div>
                    <div className="text-sm mb-3 text-slate-600 dark:text-slate-300">
                      Buy เครดิต = <b>payment_id=4</b>, Sell เครดิต = <b>payment_id=2</b> (ระบบ BE จะคำนวณ due/postponed ให้) :contentReference[oaicite:3]{index=3}
                    </div>
                    <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
                      <div>
                        <label className="mb-1 block text-sm text-slate-700 dark:text-slate-300">ระยะอนุมัติ (วัน)</label>
                        {!editing ? (
                          <div className="rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-base dark:border-slate-700 dark:bg-slate-700/60">
                            {draft.dept_allowed_period === "" ? "-" : draft.dept_allowed_period}
                          </div>
                        ) : (
                          <input
                            inputMode="numeric"
                      ...
                      <div>
                        <label className="mb-1 block text-sm text-slate-700 dark:text-slate-300">เลื่อนกำหนด (postpone)</label>
                        {!editing ? (
                          <div className="rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-base dark:border-slate-700 dark:bg-slate-700/60">
                      ...
                      <div>
                        <label className="mb-1 block text-sm text-slate-700 dark:text-slate-300">เลื่อนเพิ่ม (วัน)</label>
                        {!editing ? (
                          <div className="rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-base dark:border-slate-700 dark:bg-slate-700/60">
                            {draft.dept_postpone_period === "" ? "-" : draft.dept_postpone_period}
                          </div>
                        ) : (
                          <input
                            inputMode="numeric"
                  ...
                  {/* BUY fields */}
                  <div className="mb-6 rounded-2xl border border-emerald-200 bg-emerald-50/60 p-4 dark:border-emerald-400 dark:bg-emerald-900/10">
                    <div className="mb-2 text-base font-semibold text-emerald-800 dark:text-emerald-200">🛒 รายละเอียด (ซื้อ)</div>
                    <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
                      {[
                        ["order_serial","เลขที่ใบสำคัญ"],
                        ["entry_weight","น้ำหนักขาเข้า (กก.)","decimal"],
                        ["exit_weight","น้ำหนักขาออก (กก.)","decimal"],
                        ["weight","น้ำหนักสุทธิ (กก.)","decimal"],
                        ["price_per_kilo","ราคาต่อกก.","decimal"],
                        ["price","เป็นเงิน","decimal"],
                        ["gram","กรัม (int)","int"],
                        ["humidity","ความชื้น (%)","decimal"],
                        ["impurity","สิ่งเจือปน (%)","decimal"],
                      ].map(([k,label,type]) => (
                        <div key={k}>
                          <label className="mb-1 block text-sm text-slate-700 dark:text-slate-300">{label}</label>
                          {!editing ? (
                            <div className="rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-base dark:border-slate-700 dark:bg-slate-700/60">
                              {draft?.[k] === "" || draft?.[k] === null || draft?.[k] === undefined ? "-" : draft?.[k]}
                            </div>
                          ) : (
                            <input
                              className={baseField}
                              value={draft?.[k] ?? ""}
                              onChange={(e) => {
                                const v = type === "int" ? onlyDigits(e.target.value)
                                  : type === "decimal" ? cleanDecimal(e.target.value)
                                  : e.target.value
                                setD({ [k]: v }); touch(k)
                              }}
                              placeholder={label}
                  ...
                  {/* SELL fields */}
                  <div className="mb-2 rounded-2xl border border-amber-200 bg-amber-50/60 p-4 dark:border-amber-400 dark:bg-amber-900/10">
                    <div className="mb-2 text-base font-semibold text-amber-800 dark:text-amber-200">🧾 รายละเอียด (ขาย)</div>
                    <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
                      {[
                        ...
                        ["weight_1","น้ำหนักเที่ยว 1 (กก.)","decimal"],
                        ["weight_2","น้ำหนักเที่ยว 2 (กก.)","decimal"],
                        ["price_per_kilo","ราคาต่อกก.","decimal"],
                        ["price_1","ราคาเที่ยว 1","decimal"],
                        ["price_2","ราคาเที่ยว 2","decimal"],
                        ["gram","กรัม (int)","int"],
                      ].map(([k,label,type]) => (
                        <div key={k}>
                          <label className="mb-1 block text-sm text-slate-700 dark:text-slate-300">{label}</label>
                    ...
                    <div className="mt-3 text-sm text-slate-600 dark:text-slate-300">
                      * ฟิลด์ "ขาย" จะถูกส่งไปเฉพาะที่คุณแก้จริง ๆ (เราติดตามด้วย <code>touched</code>) เพื่อไม่ไปทับค่าอื่นโดยไม่ตั้งใจ
                    </div>
                  </div>
                </>
              )}
