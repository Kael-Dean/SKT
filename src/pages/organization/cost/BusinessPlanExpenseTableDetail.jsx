...
const SECTION_CODE = "3"
const SECTION_LABEL = "ค่าใช้จ่ายเฉพาะ ธุรกิจจัดหาสินค้า"
...
const BusinessPlanExpenseTableDetail = (props) => {
  ...
  const resetAll = async () => {
    ...
    if (effectivePlanId && effectiveBranchId && unitCols.length && unitCols[0].id !== 0) {
                ...
                if (r.kind === 'section') {
                    return (
                        <tr key={r.code} className="bg-slate-200 dark:bg-slate-700">
                          ...
                          <td className="border border-slate-300 px-2 py-2 font-extrabold text-xs dark:border-slate-600 sticky left-[60px] top-[52px] z-[15] bg-slate-200 dark:bg-slate-700">{r.label}</td>
                      <td colSpan={MONTHS.length * unitCols.length + unitCols.length + 1} className="sticky top-[52px] z-[15] bg-slate-200 dark:bg-slate-700 border-b border-slate-300 dark:border-slate-600" />
                        </tr>
                    )
                }
